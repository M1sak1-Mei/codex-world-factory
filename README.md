...
*A modular procedural fantasy-world factory built on the LAAS WebGPU world engine.*
...
Codex World Factory is a fully procedural 4×4 km open world running in the browser on WebGPU: three.js `WebGPURenderer` with TSL materials and raw WGSL compute, TypeScript strict with zero `any`, no WebGL fallback. The entire world is reproducible from a single URL parameter (`?seed=N`).

## Origin and current direction

This project started from the open-source `Braffolk/fable5-world-demo`
repository and preserves its original history, brief, and attribution. The
current fork is maintained as **Codex World Factory**: a modular system for
batch-generating terrain, ecosystems, ruins, buildings, roads, settlements,
and complete fantasy worlds.

The inherited [PROJECT_LAAS_v2.md](PROJECT_LAAS_v2.md) remains the visual and
technical baseline. New work adds data-driven Gaussian terrain recipes,
terrain-independent feature libraries, reusable procedural model kits, and a
maintainable generator/runtime boundary.
...
## Gaussian terrain recipes

The optional `?terrain=` parameter layers seed-jittered anisotropic Gaussian
stamps into macro synthesis before erosion and drainage carving. Available
recipes are `laas`, `folded-ranges`, `rift-valley`, and `caldera-lake`:

```text
http://localhost:5173/?terrain=folded-ranges&seed=42&preset=low
```

Generate a deterministic batch manifest with `npm run terrain:batch`, then
render it through the real WebGPU world with `npm run terrain:shoot`. See
